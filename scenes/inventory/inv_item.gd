...
func load_item(a_itemID : int) -> void:
	var icon_path = Inv_DataHandler.item_data[str(a_itemID)]["Sprite_Path"]
...
func _snap_to(destination: Vector2):
	...
	if int(rotation_degrees) % 180 == 0:
		destination += icon_rect_path.size/2
	else:
		var temp_xy_switch = Vector2(icon_rect_path.size.y, icon_rect_path.size.x)
